...
export function EditTask(props){
    let actualDate = props.editingTask[1]
    const year = String(actualDate.getFullYear());
    const month = String(actualDate.getMonth());
    const day = String(actualDate.getDate());
    let task = {...props.data[year][month][day][props.editingTask[0]]}
    const [hour, setHour] = useState(String(parseInt(task.time/60)).padStart(2, "0"))
    const [minute, setMinute] = useState(String(task.time-parseInt(task.time/60)*60).padStart(2, "0"))
    ...
    function send(event){
        ...
        newData[year][month][day][props.editingTask[0]] = {...value}
        props.setDataState(newData)
